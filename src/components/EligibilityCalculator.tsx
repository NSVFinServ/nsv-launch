...
import { eligibilityAPI } from '../lib/api.ts';  // still here (even if unused now)
...
const EligibilityCalculator = () => {
  ...
  const [isSubmitting, setIsSubmitting] = useState(false); // stays, UI same
  ...
  // handleContactSubmit no longer used for this flow, so you can delete it if you want
  const handleContactSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    // old API submit logic can stay commented or unused if client doesn't want it
              ...
              <div className="bg-gray-50 rounded-xl p-6">
                ...
                {showContactForm && result && !result.error && (
                  <div className="mt-8 p-6 bg-blue-50 rounded-lg border">
                    ...
                    {/* Prevent default submit – we only navigate */}
                    <form
                      className="space-y-4"
                    ...
                    >
                      ...
                      <Link
                        to="/askexpert"
                        state={{
                          fullName: contactData.name || inputs.name.trim(),
                          phone: contactData.phone || inputs.mobile.trim(),
